...
export default function DataGridDemo({marketId}) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchLastVersionsAgrement(marketId);
        setRows(data);
        console.log(data);
      } catch (error) {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div>
          <IconButton
            color="info"
            onClick={(event)=>{event.stopPropagation();downloadFile(params.row.agrementId);}}
          >
            <DownloadRoundedIcon />
          </IconButton>
        </div>
    ...
    <>
      ...
      >
        <DataGrid
          ...
          onRowClick={(rowData) => {
            navigate("/DG/agrementDetails", {
              state: { agrement: rowData.row },
            });
          }}
          sx={{
