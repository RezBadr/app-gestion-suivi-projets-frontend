...
export default function ProcedureHeader({ title, statusCp,handleClickModifyButton,procedureId}) {
  const theme = useTheme();
  const handleDownloadClick = ()=>{
    downloadFile(procedureId)
  }
    ...
    >
      ...
      >
        <Stack direction="row" spacing={2}>
          <IconButton>
            <ArrowDownwardIcon onClick={handleDownloadClick}
              sx={{ color: `${theme.palette.info.main}`, fontSize: "40px" }}
            />
          </IconButton>
          <IconButton onClick={handleClickModifyButton}>
            <EditNoteIcon
              sx={{ color: `${theme.palette.info.main}`, fontSize: "40px" }}
            />
          </IconButton>
