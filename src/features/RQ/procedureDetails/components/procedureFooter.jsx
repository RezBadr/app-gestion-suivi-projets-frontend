...
import { postNewProcedureVersion, checkIsLastVersion } from "../service";

export default function ProcedureFooter({
  statusCl,
  statusCp,
  procedureId,
  setProcedureVersion,
  ...
  React.useEffect(() => {
    handleCheckIsLastVersion(procedureId);
  }, [procedureId]);
  ...
  const handleConfirmClick = async () => {
    try {
      const newVersion = await postNewProcedureVersion(procedureId, file);
      setProcedureVersion((prev) => [...prev, newVersion]);
      resetForm();
    } catch (error) {
      console.log(error);
    }
